...
def test_build_before_after_summary_exact_values():
    before = SnapshotMetrics(
        ...
        invalid_values=4, invalid_dates=2, outliers=1, quality_score=72.0, powerbi_readiness=60.0,
    )
    after = SnapshotMetrics(
        row_count=98, column_count=5, missing_values=5, duplicate_rows=0, duplicate_ids=0,
        invalid_values=1, invalid_dates=0, outliers=1, quality_score=94.0, powerbi_readiness=85.0,
    )
...
def test_build_snapshot_metrics_from_real_data():
    ...
    snapshot = build_snapshot_metrics(df, profile, issues, quality_score=80.0, powerbi_readiness=70.0)
    ...
    assert snapshot.powerbi_readiness == 70.0
